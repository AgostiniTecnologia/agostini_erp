...
class FinanceReportService
{
    public function generateReport($startDate, $endDate, $companyId): array
    {
        ...
        while ($currentMonth->lte($finalMonth)) {
            ...
            $monthStart = $currentMonth->copy()->startOfMonth()->max($startDate);
            $monthEnd = $currentMonth->copy()->endOfMonth()->min($endDate);

            $revenue = $revenueAccounts->sum(fn ($a) => $a->getValuesForPeriod($monthStart, $monthEnd, 'entrada')
            );

            $expense = $expenseAccounts->sum(fn ($a) => $a->getValuesForPeriod($monthStart, $monthEnd, 'saida')
            );
